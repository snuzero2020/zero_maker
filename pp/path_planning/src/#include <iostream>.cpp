...
class Planning{
    private:
        ...
        int h=1040;
        int w=1200;
        //int h = int( (3 * global_point_distance + 2 * boundary_distance)/pixel_distance ); //맵 세로 픽셀 수
        //int w = int( (3 * global_point_distance + 2 * boundary_distance)/pixel_distance ); //맵 가로 픽셀 수
    ...
    public:
        Planning(){
            ...
            //*************
            int obs_check_dist=10;//node 주위 10 픽셀에 장애물이 있는지 체크

            for(int i=0; i<16; i++){
                for(int j=-obs_check_dist; j<=obs_check_dist; j++){
                    for(int k=-obs_check_dist; k<=obs_check_dist; k++){
                        if(current_map[global_points[i].x/pixel_distance+j][global_points[i].y/pixel_distance+k]==1){
                            global_points[i].obstacle=1;
                            break;
                        }
                    }
                }
        ...
        void calculate_global_path(){
                        ...
                        for (int j = 0; j < 4; ++j){
                            if ( (global_points[i].next_point[j]) != NULL){
                                if ( (global_points[i].next_point[j])->obstacle == 0){
                                    if( (global_points[i].next_point[j])->cost == -1 ){//cost==-1 => unvisited인듯?
                                        (global_points[i].next_point[j])->cost = current_cost + 1;//current_cost=방문한 순서
            ...
            //goal point에서 출발 
            while ( current_point -> cost != 0){ //출발점에 도달할 때까지
...
int main(int argc, char *argv[]){
    ...
    while (ros::ok()){
        ...
        rrt_ys::rrt_star star;
        rrt.init(pixel_distance, random_distance, choose_distance, rewire_distance, obstacle_check_distance, goal_point_arrive_distance);
	    
        //***********
        calculate_global_path(); //global path 경로(ex 15->14->10->..) global_path에 저장 

        for(int i=global_path.size()-1; i>0; i--){
            vector<ys::point> route=rrt.rrtstar(current_map, int(global_points[global_path[i]].x/pixel_distance), int(global_points[global_path[i]].y/pixel_distance),
                                                int(global_points[global_path[i-1]].x/pixel_distance),int(global_points[global_path[i-1]].y/pixel_distance));
